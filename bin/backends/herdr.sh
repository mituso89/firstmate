...
#                this safe for a crew pane entered through an interactive
#                `treehouse get`, where that nested shell sits under the pane's
#                top shell.
...
# any error. Mirrors tmux's pane_current_path poll used for fm-spawn.sh's
# worktree-arrival wait and relaunch placement check.
#
# `pane get`'s `.result.pane.cwd` is the pane's TOP-LEVEL shell cwd: on Herdr
# 0.9.1 it follows a `cd` run in that shell but never a subshell's (measured
# 2026-09-25), and it is the directory Herdr saves and restores the pane in.
# `.result.pane.foreground_cwd` tracks the actually running foreground
# process's cwd instead, which also follows a subshell such as an interactive
# `treehouse get` - confirmed live against a real treehouse acquisition.
