...
fm_agent_process_classify_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    *claude*|*codex*|*opencode*|*grok*|*kimi*|*rovo*|pi|pi-signed|pi-launcher|Pi|omp) printf 'agent' ;;
    ...
    agy) printf 'agent' ;;
    # devin (Devin CLI) is anchored for the same reason: its launcher execs a
    # native binary whose process name is the bare word `devin` (verified live,
    # devin 3000.6.12), and a glob would claim unrelated commands.
    devin) printf 'agent' ;;
