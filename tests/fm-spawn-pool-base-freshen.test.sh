...
test_linked_spawning_home_rejects_primary_before_refresh() {
  ...
  for returned in primary primary-alias spawning scout; do
    ...
    if [ "$returned" = scout ]; then
      ...
      # leased path before any pane exists: the spawning directory is the
      # project itself, and the repository primary (named directly or through a
      # symlink) shares the project's common git dir. The refusal names the
      # path Treehouse reported and why it was rejected.
      assert_contains "$out" "did not yield an isolated worktree" \
        "spawn did not explain its isolation refusal"
      assert_contains "$out" "resolved '" "refusal did not name the path Treehouse reported"
