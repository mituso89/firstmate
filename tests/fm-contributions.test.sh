...
case "$*" in
  ...
  'api repos/o/r/pulls/8')
    if [ -f "$FORGE/core-delay" ]; then sleep "$(cat "$FORGE/core-delay")"; fi
...
test_reservation_defers_later_url_when_the_reserve_does_not_remain() {
  local home out
  ...
  grep -F 'api repos/o/r/pulls/8' "$home/forge/calls" >/dev/null \
    && fail 'a later PR began without the observation reservation'
  ...
  pass 'a later URL waits when the observation reserve does not remain for it'
}

test_slow_read_under_the_call_bound_is_observed() {
  local home out
  home=$(new_home slow-read-under-bound)
  forge_home "$home"
  wrap_forge "$home"
  mutate_record "$home" delivery '.records[0].checked_at="2026-09-15T08:00:00Z"'
  # ~6s core read: over the old five-second cap, under the 12-second default.
  printf '6\n' > "$home/forge/core-delay"
  out=$(with_home "$home" env FM_CONTRIBUTIONS_BUDGET=25 "$ROOT/bin/fm-contributions.sh" poll) \
    || fail 'poll failed on a slow read under the default call bound'
  [ -z "$out" ] || fail "a slow read under the default call bound printed: $out"
  jq -e --arg now "$NOW" '.records[0] | .checked_at == $now and .error == null and .observation.state == "open"' \
    "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'a slow read under the default call bound left no fresh observation'
  pass 'a forge read over the old five-second cap is observed under the default call bound'
}

test_a_read_over_the_call_bound_is_unavailable() {
  local home out
  home=$(new_home slow-read-over-bound)
  forge_home "$home"
  wrap_forge "$home"
  mutate_record "$home" delivery '.records[0].checked_at="2026-09-15T08:00:00Z"'
  printf '6\n' > "$home/forge/core-delay"
  out=$(with_home "$home" env FM_CONTRIBUTIONS_BUDGET=25 FM_CONTRIBUTIONS_CALL_BOUND=3 \
    "$ROOT/bin/fm-contributions.sh" poll) || fail 'poll failed on a read over the call bound'
  [ "$out" = 'contributions: observation unavailable for https://github.com/o/r/pull/8' ] \
    || fail "a read over the call bound did not wake: $out"
  jq -e --arg now "$NOW" '.records[0].checked_at == $now
    and .records[0].error == "forge observation unavailable or changed during read"' \
    "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'a read over the call bound left no error evidence'
  pass 'a forge read over the call bound is recorded unavailable, proving the bound is load-bearing'
}

test_call_bound_validation() {
  local home out
  home=$(new_home call-bound-validation)
  forge_home "$home"
  out=$(with_home "$home" env FM_CONTRIBUTIONS_CALL_BOUND=0 "$ROOT/bin/fm-contributions.sh" poll 2>&1) \
    && fail 'a zero per-call bound was accepted'
  [ "$out" = 'fm-contributions: per-call bound must be 1..25 seconds' ] \
    || fail "a zero per-call bound gave the wrong refusal: $out"
  out=$(with_home "$home" env FM_CONTRIBUTIONS_CALL_BOUND=26 "$ROOT/bin/fm-contributions.sh" poll 2>&1) \
    && fail 'a per-call bound over the ceiling was accepted'
  [ "$out" = 'fm-contributions: per-call bound must be 1..25 seconds' ] \
    || fail "a per-call bound over the ceiling gave the wrong refusal: $out"
  out=$(with_home "$home" env FM_CONTRIBUTIONS_CALL_BOUND=abc "$ROOT/bin/fm-contributions.sh" poll 2>&1) \
    && fail 'a non-numeric per-call bound was accepted'
  [ "$out" = 'fm-contributions: invalid per-call bound' ] \
    || fail "a non-numeric per-call bound gave the wrong refusal: $out"
  pass 'the per-call bound refuses non-numeric and out-of-range values loudly'
}

test_final_url_does_not_spend_the_first_observation_allowance() {
  local home out
  home=$(new_home final-first-url)
  forge_home "$home"
  wrap_forge "$home"
  # Oldest first: a landed PR whose final observation needs no forge read, then
  # an open PR whose observation does. With the reserve equal to the whole
  # budget, settling the final row must not spend the allowance the open PR
  # needs - otherwise the poll breaks having observed nothing.
  record "$home" landed 3 merged mergeable
  mutate_record "$home" landed '.records[0].checked_at="2026-09-14T08:00:00Z"'
  mutate_record "$home" delivery '.records[0].checked_at="2026-09-15T08:00:00Z"'
  /bin/date +%s > "$home/forge/clock"
  # One second of real drift is enough to empty a whole-budget reserve, so fake
  # +%s calls deterministically: DEADLINE reads the clock and every later call
  # reads clock + 1, leaving exactly one second less than the reserve for any
  # gated row while a permitted observation still completes inside the budget.
  cat > "$home/fakebin/date" <<'SH'
#!/bin/sh
if [ "$*" = +%s ] && [ -f "$FORGE/clock" ]; then
  n=$(cat "$FORGE/date-calls" 2>/dev/null || printf 0); n=$((n + 1))
  printf '%s\n' "$n" > "$FORGE/date-calls"
  case "$n" in
    1) cat "$FORGE/clock" ;;
    *) printf '%s\n' "$(( $(cat "$FORGE/clock") + 1 ))" ;;
...
SH
  ...
  out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" poll) \
    || fail 'poll failed behind a final first URL'
  [ -z "$out" ] || fail "a poll behind a final first URL printed: $out"
  grep -F 'api repos/o/r/pulls/8' "$home/forge/calls" >/dev/null \
    || fail 'the open PR behind a final first URL was never observed'
  jq -e --arg now "$NOW" '.records[0] | .checked_at == $now and .error == null and .observation.state == "open"' \
    "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'the open PR behind a final first URL left no fresh observation'
  jq -e '.records[0] | .checked_at == "2026-09-14T08:00:00Z" and .observation.state == "merged"' \
    "$home/data/landed/contributions.json" >/dev/null \
    || fail 'the final observation was re-read or rewritten'
  pass 'a final first URL settles for free and the next URL still observes in the same poll'
}
