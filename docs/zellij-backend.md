...
A pane can still disappear between verification and the operation; downstream submit, worktree-arrival, and stale detection report that narrow race rather than treating exit 0 as success.
...
The spawn's worktree-arrival wait and relaunch placement check therefore send begin and end markers around `pwd`, capture the marked block, and join wrapped path lines.
This active probe is scoped to those spawn-time checks and is not advertised as a general live-cwd API.
...
- Reading the task pane's worktree cwd requires the spawn-time marker probe.
