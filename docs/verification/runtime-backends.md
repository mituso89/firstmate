...
| Restart | guarded named-session stop then start | Workspace, tab, pane, and labels persisted; the agent process and registration did not. On Herdr 0.9.1 a pane with a recorded native agent session also had its resume command typed into a fresh shell in the saved top-level shell directory ([devin.md](devin.md) "Herdr restore"). |
